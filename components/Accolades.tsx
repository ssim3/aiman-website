      ...
      <div className="flex flex-col gap-10 max-w-7xl mx-auto">
        <div className='text-center'>
          ...
          <p className="text-gray-600">
            Recognition for excellence in real estate service and community commitment
          </p>
        </div>
        ...
        <div>
              ...
              {accolades.map((accolade) => (
                <CarouselItem key={accolade.id} className="max-w-full md:basis-1/2 lg:basis-1/3">
                  <div className='group'>
                    ...
                    <div className={`max-w-full h-120 ${current + 1 === accolade.id || (current === count && accolade.id === 1) ? `lg:h-150` : `h-120`}  bg-gray-200 rounded-xl flex items-center justify-center mb-6 transition-all`}>
          ...
          <div className='flex justify-center mt-10 items-center gap-2 transition-all'>
            {Array.from(Array(count), (e, i) => {
              if (i + 1 === current) {
                return <span key={i} className='rounded-full bg-gray-500 p-1'></span>
              }
              return <span key={i} className='rounded-full bg-gray-300 p-1'></span>
            })}
          </div>
        </div>
      </div>
